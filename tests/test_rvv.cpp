...
int main() {
    ...
    // ---- vmacc.vx：vd += 純量 * 向量 ----
    // v3 初始 [1,1,1,1]（先載入），x7=3，v1=[10,20,30,40]
    // 結果 v3 = [1+3*10, 1+3*20, ...] = [31,61,91,121]，檢查 [1]=61
    check_mem("vmacc.vx [1]=1+3*20=61",
        { addi(1,0,4), vsetvli(0,1, VSEW_32|VLMUL_1),
          u(LUI,10,1), vle32(3, 10),                     // v3 <- [1,1,1,1]（0x1000）
          addi(11,0,0x100), u(LUI,12,1), add(11,11,12),
          vle32(1, 11),                                  // v1 <- [10,20,30,40]（0x1100）
          addi(7,0,3),                                   // x7 = 3
          vmacc_vx(3, 7, 1),                             // v3 += 3 * v1
          u(LUI,13,2), vse32(3, 13),
          ecall() },
        0x2004, 61,
        {{0x1000,1},{0x1004,1},{0x1008,1},{0x100c,1},
         {0x1100,10},{0x1104,20},{0x1108,30},{0x110c,40}});


    // ---- 自訂指令 laddi.pi：載入 + 指標遞增 ----
    // x20 指向 0x1000（值 77），laddi.pi 後 x7=77 且 x20=0x1004
    check("laddi.pi 載入值",
        { u(LUI,20,1), laddi_pi(7, 20),           // x7 = mem[0x1000]
          add(1,7,0), ecall() }, 1, 0);           // 佔位（下面用 check_mem 驗指標）
    // 用記憶體驗證指標遞增：laddi 兩次，第二次讀到 0x1004 的值
    check_mem("laddi.pi 指標前進",
        { u(LUI,20,1), laddi_pi(7,20),            // 讀 0x1000，x20->0x1004
          laddi_pi(8,20),                          // 讀 0x1004
          u(LUI,13,2), s(STORE,0x2,13,8,0),        // mem[0x2000]=x8
          ecall() },
        0x2000, 99, {{0x1000,77},{0x1004,99}});

    // ---- 自訂指令 vlmacc.pi：載入向量+乘加+指標遞增 ----
    // v3=0, x21->0x1100=[10,20,30,40], x7=3
    // vlmacc.pi: v3 += 3*[10,20,30,40]=[30,60,90,120], x21->0x1100+N*4
    check_mem("vlmacc.pi [2]=3*30=90",
        { addi(1,0,4), vsetvli(0,1, VSEW_32|VLMUL_1),
          addi(9,0,16), setstride(9),              // stride=16
          vsub_vv(3,3,3),                          // v3=0
          addi(21,0,0x100), u(LUI,12,1), add(21,21,12), // x21=0x1100
          addi(7,0,3),
          vlmacc_pi(3, 21, 7),                     // v3 += 3*B
          u(LUI,13,2), vse32(3,13),
          ecall() },
        0x2008, 90,
        {{0x1100,10},{0x1104,20},{0x1108,30},{0x110c,40}});

    std::cout << "\n=== RVV+custom W2~W6 通過 " << g_pass << " / "
              << (g_pass+g_fail) << " ===\n";
    return g_fail == 0 ? 0 : 1;
}
